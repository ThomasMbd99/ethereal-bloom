        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {collections.map((col, i) => (
              <motion.div key={col.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }}>
                <button
                  onClick={() => handleCollectionClick(col.id)}
                  className="group block w-full text-left aspect-video rounded-lg overflow-hidden relative"
                  style={{ background: `linear-gradient(145deg, ${col.colors.accent}18, ${col.colors.accent}05, hsl(0 0% 7%))` }}
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center group-hover:scale-[1.02] transition-transform duration-500">
                    <h2 className="font-display text-4xl lg:text-5xl tracking-wider mb-3" dangerouslySetInnerHTML={{ __html: col.displayName }} />
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-24 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="border border-border rounded-lg p-8 text-center bg-card/50 hover:border-primary/30 transition-colors duration-300">
                ...
                <p className="font-body text-sm text-muted-foreground leading-relaxed">Le format idéal pour découvrir nos créations.</p>
              </div>
              <div className="border border-primary/30 rounded-lg p-8 text-center bg-card/50 relative overflow-hidden">
                ...
                <p className="font-body text-sm text-muted-foreground leading-relaxed">Verre lourd, finitions dorées, conçu pour durer.</p>
              </div>
              <div className="border border-border rounded-lg p-8 text-center bg-card/50 hover:border-primary/30 transition-colors duration-300">
                <p className="font-display text-4xl text-primary mb-2">50ml</p>
                ...
                <p className="font-body text-sm text-muted-foreground leading-relaxed">Rechargez votre flacon signature. ♻️</p>
              </div>
            </div>
          </motion.div>
        </div>
